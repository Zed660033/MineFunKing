...
const invertedPacketsIn = Object.fromEntries(
  Object.entries(packetsIn).map(([key, value]) => [value, key])
);
const invertedPacketsOut = Object.fromEntries(
  Object.entries(packetsOut).map(([key, value]) => [value, key])
);
  ...
  cheatInterval = setInterval(() => {
    ...
    try {
      if (window?.hooked?.gameWorld?.threeScene?.scene?.fog) {
        _assign(window.hooked.gameWorld.threeScene.scene.fog, {
          near: 9999,
          far: 10000,
        });
      }
    ...
    try {
      ...
      window.hooked.gameWorld.systemsManager.activeSystems.forEach((system) => {
        ...
        if (system?.playerShooter?.currPlayerWeaponSpec?.bulletsQueue) {
          _assign(system.playerShooter.currPlayerWeaponSpec.bulletsQueue, {
            queueStepMs: 10,
          });
        }
    ...
    try {
      if (
        typeof window.hooked.gameWorld.server.msgsToSend?._push !== "function"
      ) {
        window.hooked.gameWorld.server.msgsToSend._push =
          window.hooked.gameWorld.server.msgsToSend.push;
        window.hooked.gameWorld.server.msgsToSend.push = function () {
          // Send 15 HIT packets for one-shot kill
          //   if (arguments[0] === packetsOut.HIT && Array.isArray(arguments[1])) {
          //     for (let i = 0; i < 15; i++) this._push.apply(this, arguments);
          //   }
          if (
            arguments[0] === packetsOut.HNS_ATTACK_BLOCK &&
    ...
    try {
      let system = window.hooked.gameWorld.systemsManager.activeSystems.find(
        (x) => x?.infinityBlocks !== undefined
      );
      if (system)
        _defineProperty(system, "instantBlockBreaking", {
          get() {
            return true;
          },
          set(v) { },
    ...
    try {
      ...
      let posCorrection = Object.entries(
        window.hooked.gameWorld.server.msgsListeners
      ).find(([k, v]) => v.toString().includes("=this.player.physicsPosComp"));
      if (posCorrection)
        delete window.hooked.gameWorld.server.msgsListeners[posCorrection[0]];
    } catch { }
  }, 100);
}
...
function tpToSelectedBlock() {
  try {
    let outlineSystem =
      window.hooked.gameWorld.systemsManager.activeSystems.find(
        (x) => x.currBlockPos
      );
...
function hitAll() {
  try {
    window.hooked.gameWorld.server.players.forEach((plr) => {
      ...
      if (plr.hasOwnProperty("isBlock")) {
        // HNS
        if (plr.isHunter) return;
        window.hooked.gameWorld.server.sendData(packetsOut.HNS_ATTACK_BLOCK, [
          x,
          y + 0.1,
          z,
          0.00000001,
      ...
      if (plr.hasOwnProperty("isZombie")) {
        ...
        window.hooked.gameWorld.server.sendData(packetsOut.HIT, [
          window.hooked.gameWorld.time.localServerTimeMs,
          x,
          y + 0.1,
          z,
...
function removeFloor() {
  try {
    window.hooked.gameWorld.server.players.forEach((plr) => {
      ...
      window.hooked.gameWorld.server.sendData(packetsOut.PLACE_BLOCKS, [
        +x.toFixed(3),
        +y.toFixed(3),
        +z.toFixed(3),
        1,
...
function allowBinds() {
  ...
  return (
    document?.pointerLockElement && document?.activeElement?.tagName !== "INPUT"
  );
}
...
function keybindsLoop() {
  // Control variable: Set to true to enable key bindings, set to false to disable key bindings
  const ENABLE_KEYBINDS = true; // Enable all key binding functions temporarily
  if (allowBinds() && ENABLE_KEYBINDS) {
    // Backquote key (`) - High jump function
    if (pressedKeys["Backquote"]) {
    ...
    // Backquote key (`) - Teleport to selected block function
    ...
    //   }
    // }
    // Period key (.) - Range attack function
    ...
    // Comma key (,) - Remove floor function
